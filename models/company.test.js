...
describe("findAll", function () {
  ...
  test("works: with name filter, case insensitive", async function () {
  ...
  test("works: with multiple filters", async function () {
    let companies = await Company.findAll({ name: "2", minEmployees: 2, maxEmployees: 3 });
    expect(companies).toEqual([
      {
        handle: "c2",
        name: "C2",
  ...
  test("throws error if minEmployees > maxEmployees", async function () {
    try {
      await Company.findAll({ name: "2", minEmployees: 4, maxEmployees: 3 });
      fail();
    } catch (err) {
      expect(err instanceof BadRequestError).toBeTruthy();
    }
